...
function TextSection({ textA, textB }: TextSectionProps) {
  // Split text A into lines
  const textALines = textA.split('\n');
  
  // Format text B with line breaks
  const textBParts = textB.split('\n');
  ...
  return (
    <div className="content-stretch flex flex-col gap-[25px] items-start relative shrink-0 w-full">
      <Wrapper2 additionalClassNames="h-[80px]">
        {textALines.map((line, i) => (
          <span key={i}>
            {line}
            {i < textALines.length - 1 && <br />}
          </span>
        ))}
      </Wrapper2>
      <div className="h-[160px] relative shrink-0 w-[263px]">
        <div className="absolute font-['Montserrat:Regular',sans-serif] font-normal inset-0 leading-[16px] text-[9px] text-black text-center">
          {textBParts.map((part, i) => (
            <span key={i}>
              {part}
              {i < textBParts.length - 1 && <br />}
            </span>
          ))}
        </div>
      </div>
...
function VariablePerson({ text }: { text: string }) {
  return <Wrapper3 additionalClassNames="w-[47px]">{text}</Wrapper3>;
}
...
interface A3FrontDynamicProps {
  formData: FormData;
}

export default function A3FrontDynamic({ formData }: A3FrontDynamicProps) {
  return (
    <div className="bg-white relative w-[1191px] h-[842px]" data-name="A3-Front">
